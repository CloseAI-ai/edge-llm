use edge_core::model::ComputeBackend;
...
/// On non-WASM targets, falls back to the scalar implementation from `edge_core`.
///
...
fn matvec_ternary_wasm(packed_weights: &[u8], input: &[f32], rows: usize, cols: usize) -> Vec<f32> {
    ...
    edge_core::model::matvec_ternary_scalar(packed_weights, input, rows, cols)
}
...
mod tests {
    ...
    fn test_matvec_ternary_simd_backend() {
        use edge_core::model::quantize_to_ternary;
