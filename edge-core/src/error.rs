//! Unified error types for the Edge LLM inference engine.

use thiserror::Error;

/// Top-level error type for Edge LLM operations.
#[derive(Debug, Error)]
pub enum EdgeError {
    #[error("model error: {0}")]
...
///
/// ```
/// use edge_core::error::ModelError;
///
...
/// Convenience result type using EdgeError.
...
mod tests {
    ...
    fn test_error_display() {
        let err = EdgeError::Model(ModelError::UnsupportedArchitecture("gpt4".into()));
        assert_eq!(
            err.to_string(),
    ...
    fn test_from_tokenizer_error() {
        ...
        let err: EdgeError = tok_err.into();
    ...
    #[test]
    fn test_model_error_into_edge_error() {
        let model_err = ModelError::MissingWeights("wq".into());
        let edge_err: EdgeError = model_err.into();
        assert!(matches!(edge_err, EdgeError::Model(_)));
        assert!(edge_err.to_string().contains("wq"));
    }

    #[test]
    fn test_edge_error_is_send_sync() {
        // Compile-time assertion: EdgeError must be Send + Sync for async use
    ...
    #[test]
    fn test_edge_error_tokenizer_variant_matches() {
        // From<TokenizerError> must produce EdgeError::Tokenizer, not another variant
        let tok_err = crate::tokenizer::TokenizerError::LoadError("x".into());
        let edge_err: EdgeError = tok_err.into();
        assert!(
            matches!(edge_err, EdgeError::Tokenizer(_)),
            "expected EdgeError::Tokenizer, got {edge_err:?}"
        );
    }
}
