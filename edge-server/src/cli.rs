...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: edge-cli <model.gguf> [tokenizer.json] [options]");
        eprintln!();
        eprintln!("Options:");
        ...
        eprintln!();
        eprintln!("Examples:");
        eprintln!("  edge-cli model.gguf tokenizer.json --prompt \"Hello\"");
        eprintln!("  edge-cli model.gguf tokenizer.json --chat");
        eprintln!("  edge-cli model.gguf tokenizer.json --chat --system \"You are a pirate\"");
        std::process::exit(1);
    }
    ...
    let template = match gguf.metadata.get("tokenizer.chat_template") {
        Some(edge_loader::gguf::MetadataValue::String(tmpl)) => {
...
mod tests {
    ...
    fn test_get_arg_returns_none_for_missing_flag() {
        let args = vec!["edge-cli".to_string(), "--chat".to_string()];
    ...
    fn test_get_arg_returns_none_when_flag_is_last() {
        ...
        let args = vec!["edge-cli".to_string(), "--prompt".to_string()];
        assert_eq!(get_arg(&args, "--prompt"), None);
    }
