...
#[command(
    name = "edge-server",
    about = "Edge LLM inference server with OpenAI-compatible API"
)]
struct Args {
...
async fn main() {
    ...
    let model_name = std::env::var("MODEL_NAME").unwrap_or_else(|_| "edge-local".into());
    ...
    eprintln!("Edge server listening on {addr}");
...
mod tests {
    ...
    fn test_sse_chunk_format() {
        ...
        let role = ChatCompletionChunk::new_role("edge-1b", "id-1");
        ...
        let content = ChatCompletionChunk::new_delta("edge-1b", "id-1", Some("Hi".into()), None);
        ...
        let finish = ChatCompletionChunk::new_delta("edge-1b", "id-1", None, Some("stop".into()));
