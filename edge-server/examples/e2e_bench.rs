...
//!   cargo run -p edgellm-server --example e2e_bench --release -- --gpu
//!
//! Run speculative decoding A/B comparison:
//!   cargo run -p edgellm-server --example e2e_bench --release -- --speculative
//!
//! Use a different model via --model flag or MODEL_PATH env var:
//!   cargo run -p edgellm-server --example e2e_bench --release -- --model path/to/model.gguf
//!   MODEL_PATH=path/to/model.gguf cargo run -p edgellm-server --example e2e_bench --release
...
fn main() {
    ...
    if json_mode {
        ...
        println!();
        println!("Edge LLM E2E Benchmark");
        println!("========================================");
        println!("Date:     {date}");
