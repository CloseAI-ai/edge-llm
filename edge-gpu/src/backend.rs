...
use edge_core::model::{ComputeBackend, RawWeight, WeightFormat};
use edge_core::tensor::Tensor;
use thiserror::Error;
...
impl WebGpuBackend {
    ...
    async fn new_impl(cache_data: Option<&[u8]>) -> Result<Self, GpuError> {
        ...
        if adapter_features.contains(wgpu::Features::SHADER_F16) {
            ...
            log::info!("edge-gpu: shader-f16 extension available, enabling f16 KV cache path");
        }
        ...
        if has_subgroups {
            ...
            log::info!("edge-gpu: subgroup operations enabled");
        }
        ...
        let pipeline_cache = match try_create_wgpu_cache(&device, adapter_features, cache_data) {
            Some(wgpu_cache) => {
                log::debug!("edge-gpu: driver pipeline cache enabled");
    ...
    fn upload_f32_matrix_sharded(
        ...
        log::debug!(
            "edge-gpu: sharded {}×{} f32 matrix into {} buffer(s)",
            num_rows,
            num_cols,
            shards.len(),
        );
    ...
    pub fn upload_weights_to_gpu(
        &self,
        raw_layers: &[edge_core::model::RawLayerWeights],
        layer_norms: &[(
            &[f32],         // attn_norm
            &[f32],         // ffn_norm
        ...
        *self.gpu_weights.lock().expect("gpu_weights mutex poisoned") = Some(weights);
        log::info!(
            "edge-gpu: uploaded {} layer weights to GPU ({} persistent buffers + 16 intermediate)",
            raw_layers.len(),
            raw_layers.len() * 11 + 3
        );
    }
...
impl ComputeBackend for WebGpuBackend {
    ...
    fn silu_mul_vec(&self, gate: &[f32], up: &[f32]) -> Vec<f32> {
        ...
        {
            return edge_core::model::CpuBackend.silu_mul_vec(gate, up);
        }
        #[cfg(not(target_arch = "wasm32"))]
        {
    ...
    fn grouped_query_attention(
        ...
        if attn_softcap != 0.0 {
            return edge_core::model::cpu_grouped_query_attention(
                q,
                k_cache,
                v_cache,
                num_heads,
    ...
    fn init_gpu_kv_cache(
        ...
        let kv = if self.supports_shader_f16() {
            log::info!(
                "edge-gpu: allocating f16 KV cache ({} layers, {} seq, {} heads, {} dim) — \
                 {:.1} MB (half of f32)",
                num_layers,
                max_seq_len,
                num_kv_heads,
    ...
    fn prefill_attention_gpu(
        ...
        #[cfg(target_arch = "wasm32")]
        {
            return edge_core::model::CpuBackend.prefill_attention_gpu(
                q,
                k,
                v,
                seq_len,
    ...
    fn batched_matmul(
        ...
        #[cfg(target_arch = "wasm32")]
        {
            return edge_core::model::CpuBackend
    ...
    fn batched_rmsnorm(
        ...
        #[cfg(target_arch = "wasm32")]
        {
            return edge_core::model::CpuBackend.batched_rmsnorm(input, weight, dim, batch, eps);
        }
        #[cfg(not(target_arch = "wasm32"))]
    ...
    fn batched_dequant_matmul(&self, weight: &RawWeight, input: &[f32], batch: usize) -> Vec<f32> {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            return edge_core::model::CpuBackend.batched_dequant_matmul(weight, input, batch);
        }
        #[cfg(not(target_arch = "wasm32"))]
        {
            let num_rows = weight.num_rows;
            ...
            match weight.format {
                ...
                WeightFormat::Ternary => {
                    ...
                    edge_core::model::matvec_ternary(&weight.data, input, num_rows, cols)
                }
            }
        }
    }
...
mod tests {
    ...
    /// Run manually with: `cargo test -p edgellm-gpu -- --ignored`
    #[test]
    #[ignore]
    ...
    ///
    /// Requires a GPU adapter; run with: `cargo test -p edgellm-gpu -- --ignored`
    #[test]
    #[ignore]
    ...
    fn test_dequant_matvec_q8_0_matches_cpu() {
        use edge_loader::quantize::dequant_q8_0_block;
    ...
    fn test_dequant_q5k_matches_cpu() {
        use edge_loader::quantize::dequant_q5k_block;
    ...
    fn test_dequant_q6k_zeroed() {
        use edge_loader::quantize::dequant_q6k_block;
    ...
    fn test_batched_rope_matches_cpu() {
        use edge_core::model::apply_rope;
    ...
    fn test_prefill_attention_matches_cpu() {
        use edge_core::model::cpu_grouped_query_attention;
    ...
    fn test_dequant_q3k_zeroed() {
        use edge_loader::quantize::dequant_q3k_block;
    ...
    fn test_dequant_matvec_q5_0_matches_cpu() {
        use edge_loader::quantize::dequant_q5_0_block;
