//! Browser integration for Edge LLM.
//!
...
//!
//! ```javascript
//! import init, { EdgeEngine, webgpu_available } from './pkg/edge_web.js';
//!
//! await init();
...
//! const engine = EdgeEngine.load(bytes);
//!
...
use edge_core::chat::{ChatMessage, ChatTemplate, Role};
use edge_core::generate::Generator;
use edge_core::model::Model;
use edge_core::sampling::{self, SamplingParams};
use edge_core::tokenizer::{BpeTokenizer, Tokenizer};
use edge_gpu::WebGpuBackend;
use edge_loader::gguf::{GgufFile, MetadataValue};
use edge_loader::tokenizer::GgufVocab;
use edge_loader::weights::{
...
/// Edge LLM inference engine, exported to JS.
///
...
/// In-progress GGUF load.  Breaks the monolithic [`EdgeEngine::load`] into
...
///
/// ```javascript
/// const loader = EdgeEngine.begin_load(gguf_bytes);       // parses header only
...
pub struct EdgeLoader {
    ...
    tensor_map: HashMap<String, edge_core::tensor::Tensor>,
    raw_map: HashMap<String, edge_core::model::RawWeight>,
    loaded: usize,
    ...
    model_name: String,
    metadata_json: String,
    config: edge_core::config::ModelConfig,
}
...
impl EdgeLoader {
    ...
    /// warmup, and return a ready-to-use `EdgeEngine`.  Consumes the loader.
    ///
    ...
    /// sync `EdgeEngine.load` which can do a second f32 dequant pass.
    #[wasm_bindgen]
    pub fn finalize(self) -> Result<EdgeEngine, JsError> {
        let EdgeLoader {
            gguf,
            skip_names: _,
            tensor_map,
        ...
        if raw_layers.is_none() {
            return Err(JsError::new(
                "chunked load: mixed-quant model has unsupported tensor format. \
                 Fall back to EdgeEngine.load(bytes) which can re-read with full f32 dequant.",
            ));
        }
        ...
        if let Some(rl) = raw_layers {
            ...
            web_sys::console::log_1(
                &"edge-llm: raw quantized weights not available, using f32 path".into(),
            );
        }
        model.warmup();
...
impl EdgeEngine {
    ...
    /// responsive.
    ///
    /// Prefer this over [`EdgeEngine::load`] in the browser — the sync
    /// `load` method does ~2-3 s of uninterrupted work which Chrome flags
    /// as "not responding" on memory-pressured tabs.
    #[wasm_bindgen]
    pub fn begin_load(gguf_bytes: &[u8]) -> Result<EdgeLoader, JsError> {
    ...
    #[wasm_bindgen]
    pub fn load(gguf_bytes: &[u8]) -> Result<EdgeEngine, JsError> {
        let mut reader = Cursor::new(gguf_bytes);
        let gguf = GgufFile::parse_header(&mut reader)
            .map_err(|e| JsError::new(&format!("GGUF parse error: {e}")))?;
        let chat_template = detect_chat_template(&gguf);
        ...
        let (weights, raw_layers) = match raw_layers {
            ...
            None => {
                ...
                web_sys::console::log_1(
                    &"edge-llm: mixed-quant model; re-reading with full f32 dequant".into(),
                );
                let mut reader = Cursor::new(gguf_bytes);
    ...
    ///
    /// ```javascript
    /// const cached = localStorage.getItem('edge-pipeline-cache');
    ...
    /// if (data.length > 0) {
    ///   localStorage.setItem('edge-pipeline-cache', JSON.stringify(Array.from(data)));
    /// }
    /// ```
    #[wasm_bindgen]
    ...
    /// snapshot via [`EdgeEngine::prefill_profile_json`].
    ///
    /// Overhead when enabled: one `performance.now()` call per phase boundary
    /// (~15 per layer).  Turn off via [`EdgeEngine::disable_prefill_profiling`]
    ...
    /// continues to work using the f32 path loaded at `EdgeEngine.load()`.
    ///
    ...
    /// to be passed to `EdgeTokenizer.encode()`.
    ///
    ...
    /// `next_token()` loop so callers don't need a separate `EdgeTokenizer`.
    ///
    ...
    /// remains in edge-core for callers that want it explicitly and as
    ...
    /// call `EdgeEngine.load()` again to restore the base model.
    ///
    /// ```javascript
    ...
    pub fn merge_lora(&mut self, adapter_bytes: &[u8]) -> Result<(), JsError> {
        let adapter = edge_loader::load_lora_from_safetensors(adapter_bytes)
    ...
    pub fn merge_lora_with_alpha(
        ...
        let mut adapter = edge_loader::load_lora_from_safetensors(adapter_bytes)
            .map_err(|e| JsError::new(&format!("LoRA load error: {e}")))?;
        adapter.alpha = alpha;
        self.model
            .merge_lora(&adapter)
    ...
    /// [`EdgeEngine::output_projection`] for the inverse tail step.
    #[wasm_bindgen]
...
///
/// ```javascript
/// const loader = new EdgeProgressiveLoader('https://example.com/model.gguf');
...
/// ```
#[wasm_bindgen]
pub struct EdgeProgressiveLoader {
    url: String,
}

#[wasm_bindgen]
impl EdgeProgressiveLoader {
    /// Create a loader for the given model URL.
    #[wasm_bindgen(constructor)]
    pub fn new(url: &str) -> EdgeProgressiveLoader {
    ...
    /// as each chunk arrives, then parse and return a `EdgeEngine`.
    ///
    ...
    #[wasm_bindgen]
    pub async fn load(&self, on_progress: js_sys::Function) -> Result<EdgeEngine, JsError> {
        ...
        let weights = load_model_weights_with_progress(&gguf, &mut cursor, |current, total| {
            ...
            let msg = format!("Edge LLM: loading layer {current}/{total}");
...
/// in coordination with `EdgeEngine` to run full text-in / text-out inference.
///
/// # JS example
///
/// ```javascript
...
/// const tok = EdgeTokenizer.from_json(json);
///
...
const OPFS_MODELS_DIR: &str = "edge-models";
...
/// Get (or create) the `edge-models` subdirectory inside OPFS.
...
///
/// Creates the `edge-models` directory if it does not exist.  Overwrites any
