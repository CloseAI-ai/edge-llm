/**
 * TypeScript type definitions for the Edge LLM WASM module.
 * These map to the wasm-bindgen exports from edge-web.
 */
...
/** Initialize the Edge LLM engine (async). */
...
export declare class EdgeEngine {
  ...
  static load(ggufBytes: Uint8Array): EdgeEngine;
   ...
   * template.  Pass the result to EdgeTokenizer.encode() before generating.
 ...
 * progress, then parses and returns a EdgeEngine.
 */
export declare class EdgeProgressiveLoader {
  constructor(url: string);
  /** Fetch, stream, and parse the model. Calls onProgress as chunks arrive. */
  load(onProgress: ProgressCallback): Promise<EdgeEngine>;
}
 ...
 */
export declare class EdgeTokenizer {
  /** Load from a tokenizer.json string. */
  static from_json(json: string): EdgeTokenizer;
