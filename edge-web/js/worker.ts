/**
 * Web Worker bootstrap for running Edge LLM inference off the main thread.
 *
 * Architecture:
...
async function handleInit(_msg: InitMessage) {
  try {
    ...
    const engine = await import('../pkg/edge_web.js');
    await engine.init();

    const webgpu = engine.webgpu_available();
    initialized = true;
...
async function handleGenerate(msg: GenerateMessage) {
  ...
  try {
    ...
    for (let i = 0; i < maxTokens && !aborted; i++) {
      // In real implementation: call edge-llm WASM generate step
      tokenCount++;
